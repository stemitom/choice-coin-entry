...
class AdminForm(FlaskForm):
    username = StringField("Username", validators=[DataRequired(), Length(min=0, max=50)])
...
class VoterForm(FlaskForm):
    ssn = StringField("Social Security Number", validators=[DataRequired(), Length(min=8, max=20)])
    license_id = StringField("License Identification", validators=[DataRequired(), Length(min=5, max=15)])
    category = SelectMultipleField("Level", choices=[('newbie', 'NEWBIE'), ('master', 'MASTER'), ('emeritus', 'EMERITUS')])
    admin_key = StringField("Admin Password", PasswordField[DataRequired(), Length(min=2)])
